...
function FounderNarrative({ founder }: { founder: FounderId }) {
  const text: Record<FounderId, string> = {
    ...
    jefferson:
      "Jefferson sits at a consistent rank 3 across most measures. He is rank 2 on the verified-references column thanks to his 26 by-name references. Under the strict Shakespeare-only standard his Brutus and Caesar passages no longer count as thematic Shakespeare evidence (they're Roman names reachable through Plutarch), so the thematic-invocations measure shows him tied with the others below Adams. A balanced influence pattern with no extremes.",
    washington:
      "Washington is steady mid-pack: rank 4 or 5 across the matrix. He stays just outside the top three on the statistical measures and falls further on the evidence-based ones. The single Shakespeare borrowing his name is attached to ('band of brothers') is a small piece of a generally non-literary corpus.",
    hamilton:
      "Hamilton ranks fifth on the composite under the strict Shakespeare-only standard, one step above Madison after the false-positive audit. He is rank 6 on Shakespearean context patterns and on the statistical-style overlap column; he ties Madison at rank 5.5 on verified references (zero on both). His 1779 letter calling General Lee 'a little spice of the Julius Caesar or Cromwell' was treated as a Shakespeare exception in an earlier version of the project; the revised methodology sets it aside as a historical-Caesar reference (paired with Cromwell, a non-Shakespeare figure), not a play allusion.",
    madison:
      "Madison sits alone at the bottom of the composite at 0.17. He is rank 6 on six of the eleven methods. His one quiet outlier is the use of Shakespeare-coined phrases (rank 3), where a small number of widely-shared phrases lift him momentarily out of last place. The two MEDIUM-tier catalogue items he had previously held were both dropped during the source-level audit: the 'I love thee' direct quote was traced to a Talbot Mundy adventure novel spliced into his Gutenberg supplement, and the 1780 'tempest' reference was reclassified as LOW (it described real hurricane damage in the West Indies, not the play).",
  };
  return (
